<?php
	...
	if(empty($_REQUEST['cl']) || empty($_REQUEST['f']))
	{
		...
		$pdf->Ln();
		$pdf->image("img/logo.PNG", 3, 2, 15, 15, 'PNG');
		$pdf->image("img/logo.PNG", 60, 2, 15, 15, 'PNG');
		$pdf->SetFont('Arial', 'B', 7);
		$pdf->Cell(15, 5, "Rnc: ", 0, 0, 'L');
		$pdf->SetFont('Arial', '', 7);
		...
		$pdf->Cell(20,5,utf8_decode("Teléfono") , 0, 0, 'L');
		$pdf->Cell(15, 5, utf8_decode("Dirección"), 0, 1, 'L');
		$pdf->SetFont('Arial', '', 7);
		if ($_GET['cl'] == 1) {
		...
		$pdf->Cell(20, 5, utf8_decode($result_cliente['nombre']), 0, 0, 'L');
		...
		$pdf->SetFont('Arial', 'B', 7);
		$pdf->Cell(42, 5, utf8_decode("Descripción") , 0, 0, 'L');
		...
		$pdf->Cell(76, 5, 'Total a pagar: ' . number_format($result_venta['totalfactura'], 2, '.', ','), 1, 1, 'R');
		$pdf->Ln();
		$pdf->SetFont('Arial', '', 7);
